 ...
 * esistono già per (event_id, role_code) e si inseriscono solo `max(0, quantity - count)`
 * nuove righe DRAFT con staff_id NULL.
 */
export async function ensureAssignmentsForEvent(
  ...
  const requirementsResult = await pool.query(
    `SELECT sr.role_code, sr.quantity
  ...
  for (const reqRow of requirementsResult.rows) {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*)::int as cnt FROM assignments
       WHERE event_id = $1 AND role_code = $2`,
      [eventId, role_code]
    );
